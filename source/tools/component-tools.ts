...
export class ComponentTools implements ToolExecutor {
            ...
            let processedValue: any;
                ...
                switch (finalPropertyType) {
                    ...
                    case 'assetArray':
                        if (Array.isArray(value)) {
                            processedValue = value.map((item: any) => {
                                ...
                                if (item && typeof item === 'object' && typeof item.uuid === 'string') return { uuid: item.uuid };
                    ...
                    case 'booleanArray':
                        if (Array.isArray(value)) {
                            processedValue = value.map((item: any) => {
                                if (typeof item === 'string') return item.toLowerCase() === 'true' || item === '1';
                                return Boolean(item);
                            });
                        } else {
    ...
    private analyzeProperty(component: any, propertyName: string): { exists: boolean; type: string; availableProperties: string[]; originalValue: any; metadata?: any } {
        // 从复杂的组件结构中提取可用属性
        const availableProperties: string[] = [];
        let propertyValue: any = undefined;
        let propertyExists = false;
        let propertyMetadata: any = undefined;
    ...
    private getArrayElementCocosType(metadata: any): string | undefined {
        if (!metadata || typeof metadata !== 'object') return undefined;
        const candidates = [metadata.type, metadata.ctor, metadata.elementType, metadata.itemType];
        for (const candidate of candidates) {
            if (typeof candidate === 'string' && candidate.length > 0 && candidate !== 'Array') return candidate;
    ...
    private detectArrayPropertyType(propertyName: string, value: any[], metadata?: any): string {
        const cocosType = (this.getArrayElementCocosType(metadata) || '').toLowerCase();
        if (cocosType === 'cc.node' || cocosType.endsWith('.node')) return 'nodeArray';
        if (cocosType === 'cc.color' || cocosType.endsWith('.color')) return 'colorArray';
        if (cocosType === 'number' || cocosType === 'cc.number' || cocosType === 'float' || cocosType === 'integer') return 'numberArray';
        if (cocosType === 'boolean' || cocosType === 'cc.boolean') return 'booleanArray';
        if (cocosType === 'string' || cocosType === 'cc.string') return 'stringArray';
        if (cocosType && (cocosType.startsWith('cc.') || cocosType.includes('asset'))) return 'assetArray';

        const sample = value.find(item => item !== null && item !== undefined);
        if (typeof sample === 'number') return 'numberArray';
        if (typeof sample === 'boolean') return 'booleanArray';
        if (sample && typeof sample === 'object') {
            if ('r' in sample && 'g' in sample && 'b' in sample) return 'colorArray';
        ...
        const name = propertyName.toLowerCase();
        if (name.includes('node') || name.includes('target')) return 'nodeArray';
        if (name.includes('color')) return 'colorArray';
        return 'stringArray';
    }

    private isSupportedArrayType(type: string): boolean {
        return ['nodeArray', 'assetArray', 'colorArray', 'numberArray', 'booleanArray', 'stringArray'].includes(type);
    }

    private getArrayDumpElementType(type: string, metadata?: any): string {
     ...
     * 自动检测属性类型
     */
    private autoDetectPropertyType(propertyName: string, value: any, originalValue: any, detectedType: string, metadata?: any): string {
